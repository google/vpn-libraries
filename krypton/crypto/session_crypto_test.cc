...
TEST_F(SessionCryptoTest, TestInitialKeyGeneration) {
  ASSERT_OK_AND_ASSIGN(auto local_crypto,
                       SessionCrypto::Create(bridge_config_aes_128_));
  LOG(INFO) << "Private Key " << local_crypto->PrivateKeyTestOnly();

  EXPECT_EQ(X25519_PRIVATE_KEY_LEN,
            local_crypto->PrivateKeyTestOnly().length());

  auto local_keys = local_crypto->GetMyKeyMaterial();
  EXPECT_EQ(X25519_PUBLIC_VALUE_LEN, local_keys.public_value.length());

  EXPECT_NE(local_crypto->downlink_spi(), 0);
}
...
TEST_F(SessionCryptoTest, TestSharedKey) {
  ASSERT_OK_AND_ASSIGN(auto local_crypto,
                       SessionCrypto::Create(bridge_config_aes_128_));
  ASSERT_OK_AND_ASSIGN(auto remote_crypto,
                       SessionCrypto::Create(bridge_config_aes_128_));
...
TEST_F(SessionCryptoTest, TestSettingInvalidRemoteNonce) {
  ...
  EXPECT_THAT(
      local_crypto->SetRemoteKeyMaterial(remote_keys.public_value,
                                         remote_keys.nonce),
      StatusIs(absl::StatusCode::kFailedPrecondition, HasSubstr("Nonce")));
}
...
TEST_F(SessionCryptoTest, TestIpSecTransformParams) {
  ASSERT_OK_AND_ASSIGN(auto local_crypto, SessionCrypto::Create(ipsec_config_));
  ASSERT_OK_AND_ASSIGN(auto remote_crypto,
                       SessionCrypto::Create(ipsec_config_));
...
TEST_F(SessionCryptoTest, TestBridgeTransformParams256) {
  ASSERT_OK_AND_ASSIGN(auto local_crypto,
                       SessionCrypto::Create(bridge_config_aes_256_));
  ASSERT_OK_AND_ASSIGN(auto remote_crypto,
                       SessionCrypto::Create(bridge_config_aes_256_));
...
TEST_F(SessionCryptoTest, VerifyRekey) {
  ...
  ASSERT_OK_AND_ASSIGN(auto previous,
                       SessionCrypto::Create(bridge_config_aes_128_));
  ASSERT_OK_AND_ASSIGN(auto current,
                       SessionCrypto::Create(bridge_config_aes_128_));
...
TEST_F(SessionCryptoTest, VerifySignature) {
  ASSERT_OK_AND_ASSIGN(auto crypto,
                       SessionCrypto::Create(bridge_config_aes_128_));
  std::string data = "foo";
