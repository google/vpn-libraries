...
#include "privacy/net/krypton/datapath/fastpath.h"
...
FastPath::Forwarder::Forwarder(
    int source_fd, int destination_fd, const string& destination_ip_address,
    int port, Direction direction, utils::LooperThread* looper_thread,
...
absl::Status FastPath::Forwarder::RunInternal() {
  ...
  while (true) {
    ...
    if (!status.ok()) {
      PostDatapathFailure(status, direction_ == Direction::UPLINK
                                      ? destination_fd_
                                      : source_fd_);
      continue;
    }
    ...
    for (int i = 0; i < num_events; ++i) {
      ...
      if (EventsHelper::FileCanRead(events[i])) {
        ...
        int read_bytes;
        do {
          read_bytes = read(source_fd_, &buffer, kMaxPacketSize);
        } while (read_bytes == -1 && errno == EINTR);

        if (read_bytes <= 0) {
          PostDatapathFailure(
              absl::DataLossError(absl::Substitute(
...
void FastPath::Forwarder::PostDatapathFailure(const absl::Status& status,
  ...
  auto* notification = datapath_notification_;
  notification_thread_->Post([notification, failed_fd, status]() {
    notification->DatapathFailed(status, failed_fd);
  });
