...
class IpSecEncapDecapTest : public ::testing::Test {
 public:
  ...
  Encryptor encryptor_ = Encryptor(2);
...
TEST_F(IpSecEncapDecapTest, TestPacketsWithPaddingAreHandledCorrectly) {
  ...
  EXPECT_OK(encryptor_.Start(params_));
  EXPECT_OK(decryptor_.Start(params_));

  auto encryptedPacket = encryptor_.Process(packet);
  EXPECT_OK(encryptedPacket);
  EXPECT_NE(encryptedPacket->data(), packet.data());
  auto decryptedPacket = decryptor_.Process(encryptedPacket.value());
