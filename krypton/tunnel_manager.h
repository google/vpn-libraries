...
class TunnelManager : public TunnelManagerInterface {
 public:
  ...
  absl::StatusOr<PacketPipe*> GetTunnel(TunFdData) override
      ABSL_LOCKS_EXCLUDED(mutex_);
  ...
  PacketPipe* active_tunnel_test_only() const ABSL_LOCKS_EXCLUDED(mutex_);
 ...
 private:
  ...
  absl::optional<TunFdData> active_tunnel_data_ ABSL_GUARDED_BY(mutex_);
